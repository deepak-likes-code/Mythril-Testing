...
contract transaction_malleablity {
    ...
    constructor(address[] owners, uint256[] init) {
    ...
    function transfer(
        ...
        bytes32 txid = keccak256(
            abi.encodePacked(
                getTransferHash(_to, _value, _gasPrice, _nonce),
                _signature
            )
        );
        require(!signatureUsed[txid]);

        address from = recoverTransferPreSigned(
            _signature,
            _to,
            _value,
            _gasPrice,
    ...
    function recoverTransferPreSigned(
        ...
        return
            ecrecoverFromSig(
                getSignHash(getTransferHash(_to, _value, _gasPrice, _nonce)),
                _sig
            );
    }
